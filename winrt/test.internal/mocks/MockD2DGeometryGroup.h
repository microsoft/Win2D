...
namespace canvas
{
    class MockD2DGeometryGroup : public RuntimeClass<
        RuntimeClassFlags<ClassicCom>,
        ChainInterfaces<ID2D1GeometryGroup, ID2D1Geometry, ID2D1Resource>>
    {
    public:
        CALL_COUNTER_WITH_MOCK(GetFillModeMethod, D2D1_FILL_MODE());
        CALL_COUNTER_WITH_MOCK(GetSourceGeometryCountMethod, UINT32());
        CALL_COUNTER_WITH_MOCK(GetSourceGeometriesMethod, void(ID2D1Geometry**, UINT32));
        ...
        STDMETHOD_(void, GetSourceGeometries)(
            ...
            UINT32 geometriesCount) const override
        {
            GetSourceGeometriesMethod.WasCalled(geometries, geometriesCount);
        }

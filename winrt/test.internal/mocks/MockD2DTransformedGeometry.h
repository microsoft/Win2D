...
namespace canvas
{
    class MockD2DTransformedGeometry : public RuntimeClass<
        RuntimeClassFlags<ClassicCom>,
        ChainInterfaces<ID2D1TransformedGeometry, ID2D1Geometry, ID2D1Resource>>
    {
    public:
        CALL_COUNTER_WITH_MOCK(GetSourceGeometryMethod, void(ID2D1Geometry**));
        ...
        STDMETHOD_(void, GetSourceGeometry)(
            ID2D1Geometry** sourceGeometry) const override
        {
            GetSourceGeometryMethod.WasCalled(sourceGeometry);
        }

        STDMETHOD_(void, GetTransform)(
            D2D1_MATRIX_3X2_F* transform) const override
        {
            GetTransformMethod.WasCalled(transform);
        }

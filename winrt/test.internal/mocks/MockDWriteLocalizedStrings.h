...
namespace canvas
{
    class MockDWriteLocalizedStrings : public RuntimeClass<
        RuntimeClassFlags<ClassicCom>,
        IDWriteLocalizedStrings>
    {
    public:
        CALL_COUNTER_WITH_MOCK(GetCountMethod, uint32_t());
        CALL_COUNTER_WITH_MOCK(FindLocaleNameMethod, HRESULT(WCHAR const*, uint32_t*, BOOL*));
        ...
        STDMETHOD(FindLocaleName)(
            ...
            BOOL* exists)
        {
            return FindLocaleNameMethod.WasCalled(localeName, index, exists);
        }
